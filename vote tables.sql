...
CREATE TABLE if not exists voter_table(
  Identifier char(15) NOT NULL unique,
...
CREATE TABLE if not exists party_table(
  ...
  LeaderIdentifier char(15) not null unique,
  CONSTRAINT PK_PARTY PRIMARY KEY (PartyId),
  CONSTRAINT FK_VOTER_3 FOREIGN KEY (LeaderIdentifier) references voter_table(Identifier)
);
...
CREATE TABLE if not exists vote_table(
  ...
  CONSTRAINT FK_VOTERID FOREIGN KEY (Identifier) references user_table(Identifier),

...
update voter_table
set Phone = 9283478293
where Identifier = "5773 7940 7366";
...
SELECT Result.PartyID,
    party_table.PartyName,
    SUM(Vote_Count) AS 'Total_Count'
FROM Result,
    party_table
...
from Voter_Table,
    User_Table
where Voter_Table.Identifier = User_table.Identifier
    and User_Table.isActive = 0;
#OR 
delete Voter_Table,
User_Table
From Voter_Table
    INNER JOIN User_Table on Voter_Table.Identifier = User_table.Identifier
where User_Table.isActive = 0;
select count(*)
from user_table
where User_Table.isActive = 0;
insert into User_Table
values("UDJ092345", "UDJ@0192", 0, "8392 3921 0192", 2);
...
FROM candidate_table
    LEFT JOIN voter_table ON candidate_table.Identifier = voter_table.Identifier
    INNER JOIN party_table ON Party_table.Partyid = candidate_table.partyID;
